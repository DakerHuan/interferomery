...
void threadGetCapture(LPVOID lpParameter)
{
	...
	while (true)
	{
		...
		if (!stop)
		{
			...
			resize(readframe, srcframe, Size(720, 540), 0, 0, INTER_LINEAR);
			...
			{
				//pDlg->DrawResult(srcframe);
				if (TaskDispose::backgroundCircle == 0)
				{
					if (TaskDispose::statechange == 1)
					{
						TaskDispose::statechange = 0;
						startCount = 0;
						startCaulationCenter = 0;
						center.x = 0;
						center.y = 0;
						radius = 0;
						CountNumer = 0;
						CountNumer1 = 0;
						workMask1.setTo(0);  //放置工作盘掩模图
						workMask2.setTo(0);//放置工作盘掩模图的腐蚀图（去掉梯度检测后的工作盘最外圈的圆环）
						result = true;
						CountC = 0;
						StageCenterPoint.x = 0;
						StageCenterPoint.y = 0;
					...
					startCount++;
					if (startCount < 100) continue;  //100帧之后开始处理 避免前期没必要处理的帧
					if (startCount > 99999999) startCount = 101;


					cvtColor(srcframe, tmpMat, CV_RGB2GRAY);

					Mat resultMat = tmpMat.clone();

					/********************************得到粗略的中心点+中心工作圆盘的mask掩模图+此过程只计算一次：start*****************************************/
					startCaulationCenter++;
					if (startCaulationCenter == 1)
					{
						threshold(tmpMat, tmpMat, 100, 255, THRESH_BINARY);
						...
						Rect ccomp;
						floodFill(workMask1, seed, newVal, &ccomp, Scalar(20), Scalar(20), 4);
						Mat element1 = getStructuringElement(MORPH_ELLIPSE, Size(30, 30));   //距离圆形工作盘外边缘15行之内的条纹不检
						morphologyEx(workMask1, workMask2, MORPH_ERODE, element1);           //腐蚀边界没有填充部分
						workMask1 = ~workMask1;
					...
					//[3]梯度检测提取干涉条纹特征
					addWeighted(resultMat, 1, workMask1, -1, 0, tmpMat);       //背景黑化
					Mat	element = getStructuringElement(MORPH_ELLIPSE, Size(8, 8));
					morphologyEx(tmpMat, tmpMat, MORPH_GRADIENT, element);
					threshold(tmpMat, tmpMat, 110, 255, THRESH_BINARY);
					bitwise_and(tmpMat, workMask2, tmpMat);       //背景黑化

																  //[4]blob分析
					int beiji_count = 0;
					vector<vector<Point2i>> beiji_blob;
					pDlg->find_beiji_Region(tmpMat, 200, 1, 1, beiji_count, beiji_blob);


					if (beiji_blob.size() >= 2 || (beiji_blob.size() == 1 && beiji_blob[0].size() > 10000))   //少于三条条纹的图片不捡
					{
						threshold(resultMat, tmpMat, 150, 255, THRESH_BINARY);
						/***********************************噪声消除*****************************************/

						pDlg->find_beiji_Region(tmpMat, 400, 1, 1, beiji_count, beiji_blob);
						tmpMat.setTo(0);
						...
						Rect bouding = boundingRect(tmpMat);
						//center.x = bouding.x + bouding.width / 2;
						//center.y = bouding.y + bouding.height / 2;
						//radius = min(bouding.width, bouding.height) / 2;
						////circle(src_Image, center, cvRound(radius), Scalar(0, 0, 255), 2, LINE_AA);
						//srcframe.copyTo(waitforprocframe);
						//Point PtMapCenter;
						//PtMapCenter.x = center.x - TaskDispose::origin.x;
						//PtMapCenter.y = center.y - TaskDispose::origin.y;
						//circle(tempPtMap, PtMapCenter, radius, Scalar(1), -1);
						...
						//waitKey(10);
						//continue;
						int tmpradius = min(bouding.width, bouding.height) / 2;
						...
						if (center.x == 0 && radius == 0)
						{
							...
							PtMapCenter.x = center.x - TaskDispose::origin.x;
							PtMapCenter.y = center.y - TaskDispose::origin.y;
							circle(tempPtMap, PtMapCenter, radius*0.95, Scalar(1), -1);
							tempPtMap.copyTo(TaskDispose::PtMap);
							if (startCount % 50 == 0)
							{
								pDlg->StaticAnaysis();
						...
						if (center.x != 0 && radius < tmpradius)
						{
							center.x = bouding.x + bouding.width / 2;
							center.y = bouding.y + bouding.height / 2;
							radius = min(bouding.width, bouding.height) / 2;
					...
					else
					{
						pDlg->DrawResult(srcframe);
						resize(srcframe, showsrcframe, Size(pDlg->showimgsizex, pDlg->showimgsizey), 0, 0, INTER_LINEAR);
						imshow("MyMainShowWnd", showsrcframe);  //显示当前帧
						waitKey(10);
						continue;
					}
				}
				else
				{
					...
					if (startCaulationCenter == 1)
					{
						...
						morphologyEx(workMask1, workMask2, MORPH_ERODE, element1);           //腐蚀边界没有填充部分
						workMask1 = ~workMask1;
						center.x = ccomp.x + ccomp.width / 2;
						center.y = ccomp.y + ccomp.height / 2;
					}
					/********************************得到粗略的中心点+中心工作圆盘的mask掩模图+此过程只计算一次：end*****************************************/
					...
					if (beiji_blob.size() >= 2 || (beiji_blob.size() == 1 && beiji_blob[0].size() > 10000))   //少于三条条纹的图片不捡
					{
						...
						int tmpRdius;
						flag = 0;
						result = pDlg->drawfringe(tmpMat, beiji_blob, CountNumer, tmpCter, tmpRdius, radius, center, StageCenterPoint, flag);
						if (result == false)
						{
							pDlg->DrawResult(srcframe);
							resize(srcframe, showsrcframe, Size(pDlg->showimgsizex, pDlg->showimgsizey), 0, 0, INTER_LINEAR);
						...
						/**************************************start:保存当前状态下的中心值******************************************************/
						if (flag == 1 && CountNumer > 20 && num == 0)    //flag为1  开始出现偏离当前转态的位置出现
						{
							a = 1;   // 此时a置1   开始计数 
							num = 1; //计数开关
							aaa.clear();
						...
						if (a == 1 && num == 1)
						{
							...
							if (aaa.size() == 10)
							{
								...
								{
									aaaaa = 1;   //连续10张中出现有5张中心点异变情况开始更新新的中心点
									a = 0;
								}
								num = 0;
							}
						...
						if (CountNumer1 < 20 && CountNumer>20 && aaaaa == 1 && NumCenterPoint[19].x == 0)   //重新选取20张新图作为新的中心点计算
						{
							NumCenterPoint[CountNumer1].x = tmpCter.x;
							NumCenterPoint[CountNumer1].y = tmpCter.y;
							CountNumer1++;
							pDlg->DrawResult(srcframe);
							resize(srcframe, showsrcframe, Size(pDlg->showimgsizex, pDlg->showimgsizey), 0, 0, INTER_LINEAR);
							imshow("MyMainShowWnd", showsrcframe);
						...
						CenterPoint tmpCIR1;
						if (CountNumer > 21 && NumCenterPoint[19].x != 0)
						{
							for (int i = 0; i < 20 - 1; i++)
							{
								for (int j = 0; j < 20 - 1 - i; j++)
								{
									if (NumCenterPoint[j].x < NumCenterPoint[j + 1].x)
									{
										tmpCIR1 = NumCenterPoint[j];
										NumCenterPoint[j] = NumCenterPoint[j + 1];
										NumCenterPoint[j + 1] = tmpCIR1;
									}
								}
							}
							StageCenterPoint.x = (NumCenterPoint[9].x + NumCenterPoint[10].x) / 2;
							StageCenterPoint.y = (NumCenterPoint[9].y + NumCenterPoint[10].y) / 2;
							center = StageCenterPoint;          //新位置中心点更新
							NumCenterPoint[19].x = 0;
							CountNumer1 = 0;
							aaaaa = 0;
							pDlg->DrawResult(srcframe);
							resize(srcframe, showsrcframe, Size(pDlg->showimgsizex, pDlg->showimgsizey), 0, 0, INTER_LINEAR);
						...
						/*********************求取20张离粗略中心点很近的20个符合要求的内切圆+只计算一次：start*********************/
						if (CountNumer < 20)
						{
							if (abs(tmpCter.x - center.x) < 150 && abs(tmpCter.y - center.x) < 150)
							{
								tmpCirPoint[CountNumer].x = tmpCter.x;
								tmpCirPoint[CountNumer].y = tmpCter.y;
								tmpCirPoint[CountNumer].radius = tmpRdius;
								CountNumer++;
							}
							pDlg->DrawResult(srcframe);
							resize(srcframe, showsrcframe, Size(pDlg->showimgsizex, pDlg->showimgsizey), 0, 0, INTER_LINEAR);
						...
						{
							CenterPoint tmpCIR;
							CenterPoint tmpSortX[16];
							CenterPoint tmpSortY[12];
							CenterPoint tmpSortRadius[12];
							...
							for (int i = 0; i < 20 - 1; i++)
							{
								for (int j = 0; j < 20 - 1 - i; j++)
								{
									if (tmpCirPoint[j].x < tmpCirPoint[j + 1].x)
									{
										tmpCIR = tmpCirPoint[j];
										tmpCirPoint[j] = tmpCirPoint[j + 1];
										tmpCirPoint[j + 1] = tmpCIR;
									}
								}
							}
							for (int i = 0; i < 16; i++)
							{
								tmpSortX[i] = tmpCirPoint[i + 2];			 //剔除两头各两个			
							}
							center.x = (tmpSortX[7].x + tmpSortX[8].x) / 2;  //取中值得到中心点点的X坐标
							...
							for (int i = 0; i < 16 - 1; i++)
							{
								...
								{
									if (tmpSortX[j].y < tmpSortX[j + 1].y)
									{
										tmpCIR = tmpSortX[j];
										tmpSortX[j] = tmpSortX[j + 1];
										tmpSortX[j + 1] = tmpCIR;
									}
								}
							}
							for (int i = 0; i < 12; i++)
							{
								tmpSortY[i] = tmpSortX[i + 2];            //剔除两头两个
							}
							center.y = (tmpSortY[5].y + tmpSortY[6].y) / 2;  //取中值得到中心点点的X坐标
							...
							for (int i = 0; i < 12 - 1; i++)
							{
								...
								{
									if (tmpSortY[j].radius < tmpSortY[j + 1].radius)
									{
										tmpCIR = tmpSortY[j];
										tmpSortRadius[j] = tmpSortY[j + 1];
										tmpSortY[j + 1] = tmpCIR;
									}
								}
							}
							radius = tmpSortY[0].radius;         //得到最大半径
						}
						CountNumer++;
						if (CountNumer > 99999)                    //避免值过大内存不够
						{
							CountNumer = 21;
						}

						if (CountNumer > 20 && radius != 0)  //20个清晰条纹处理完毕后，一旦有条纹出现显示圆圈
						{
							srcframe.copyTo(waitforprocframe);
							Point PtMapCenter;
							PtMapCenter.x = center.x - TaskDispose::origin.x;
							...
							if (startCount % 50 == 0)
							{
								...
								long tim = finish - start;
								char t[256];
								String strright;

								sprintf_s(t, "%9d", tim);
								strright = t;
								const char* showtext = strright.c_str();
								putTextZH(srcframe, showtext, cvPoint(260, 15), Scalar(0, 200, 200), 30, "楷体", false, false);*/
							}
							circle(srcframe, center, cvRound(radius), Scalar(0, 0, 255), 2, LINE_AA);
							pDlg->DrawResult(srcframe);
							resize(srcframe, showsrcframe, Size(pDlg->showimgsizex, pDlg->showimgsizey), 0, 0, INTER_LINEAR);
					...
					if (showcircleindex == 153)
					{
						...
						for (int i = 0; i < automulticontoursrebuildcut.size(); i++)
						{
							...
							if (radius > 8 && contourcenter.x > radius&&contourcenter.y > radius && ((dis + radius) < TaskDispose::MultiCtrRadius))
							{
								...
								Mat cutimg128;
								resize(cutimg, cutimg128, Size(128, 128), 0, 0, INTER_LINEAR);
								...
								Mat tempPtMap128;
								resize(tempPtMap, tempPtMap128, Size(128, 128), 0, 0, INTER_LINEAR);
