...
bool ContastInhence(Mat &src, Mat &dst)
{
	if (src.empty()) return false;
	if (src.cols != dst.cols || src.rows != dst.rows) return false;

	//参数准备
	int rowNumber = dst.rows;  //行数
	int colNumber = dst.cols*dst.channels();  //列数 x 通道数=每一行元素的个数

	//双重循环，遍历所有的像素值
	for (int i = 0; i < rowNumber; i++)  //行循环
	{
		uchar* data = src.ptr<uchar>(i);  //获取第i行的首地址
		uchar* data1 = dst.ptr<uchar>(i);  //获取第i行的首地址
		...
		for (int j = 0; j < colNumber; j++)   //列循环
		{
			...
			if (data[j] <= 90)
			{
				data1[j] = data[j] * data[j] * 50 / 4900;
				continue;
			}

			//if (data[j] > 130 && data[j] < 255)
			//{
			//	data1[j] = ceil((double)(255 * exp((double)(0.0045*data[j])) / (double)(exp(0.0045 * 255))));
			//	continue;
			//}
			data1[j] = data[j];
		}
...
bool drawfringe(Mat &tmpMat, vector<vector<Point2i>> beiji_blob, int CountNumer, Point &Currentcenter, int &Currentradius, int &radius, Point &center, Point &StageCenterPoint, int&flag, Mat &src_Image, Mat &workMask2,bool addway)
{
	...
	if (Currentradius > radius-3 && (abs(bouding.center.x - StageCenterPoint.x) + abs(bouding.center.y - StageCenterPoint.y)) < 35)
	{
		//maxCircleUpdata(src_Image, workMask2, radius, center);
		center = Currentcenter;
		if (!addway)
		{
			radius = radius + (Currentradius - radius) / 2-2;

		}
		else
		{
			radius = radius + (Currentradius - radius) / 2;
	...
	while (true)
	{
		...
		if (!stop)
		{
			...
			else if (TaskDispose::aperModel == 0)
			{
				...
				Mat currentImg(tmpMat.size(), CV_8UC1);
				ret=ContastInhence(tmpMat, currentImg);
				if (!ret) tmpMat.copyTo(currentImg);
				...
				//【3】梯度检测提取干涉条纹特征
				if (abs(MaxCircleRadius - radius) < 15|| MaxCircleRadius<radius )
				{
					addway = false;
				}
